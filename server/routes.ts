...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/calendar/auth", async (req, res) => {
    try {
      ...
      const calendarPassword = "CalendarFY26"; // Hardcoded for now, could be moved to env
      
      if (password === calendarPassword) {
        req.session.calendarAccess = true;
        res.json({ success: true, message: "Calendar access granted" });
      } else {
        res.status(401).json({ error: "Incorrect calendar password" });
      }
    } catch (error) {
  ...
  // Calendar access route - redirects to external TeamTime app
  app.get("/api/calendar", (req, res) => {
    if (req.session.calendarAccess) {
      res.redirect("https://team-time-evarussell4.replit.app");
    } else {
      res.status(403).json({ error: "Calendar access denied" });
    }
  });
